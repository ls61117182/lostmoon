...
assert.match(menu, /const unlocked = meta\.entryKind === 'random'[\s\S]*?MenuProgress\.isUnlocked/);
assert.match(menu, /meta\.entryKind !== 'random' && !MenuProgress\.isUnlocked/);
assert.match(menu, /generateRandomMissionPackage\(meta\.randomTheater, Date\.now\(\)\)/);
