...
assert.match(scene, /private applyMainGunAttackResult[\s\S]*?hadATGunCrew && target\.destroyed[\s\S]*?this\.releaseATGunCrew\(target\)/, 'AP destruction should release the surviving AT-gun crew');
assert.doesNotMatch(scene.match(/private applyMainGunAttackResult[\s\S]*?\n  }\n\n/)?.[0] ?? '', /applyInfantrySuppression/, 'AP destruction must not suppress the surviving crew');
...
assert.match(scene, /private rollHighExplosiveCollateralResults[\s\S]*?isAntiTankGunUnit\(target\)[\s\S]*?unit\.attachedToATGunId === target\.id/,
  'HE against an AT gun should add one independent infantry-rule roll for its attached crew');
...
assert.match(scene, /private drawATGunCrewMaybeAnim\(gun: Unit\)[\s\S]*?const crew = this\.atGunCrewActor\(gun\)[\s\S]*?drawInfantry\(crew[\s\S]*?drawSuppressionMarks\(crew, c\.x, c\.y, offsets\)/,
  'a suppressed attached AT-gun crew should draw exclamation marks at its visible operator positions');
assert.match(scene, /private drawSuppressionMarks\([\s\S]*?formationOffsets\?[\s\S]*?const offsets = formationOffsets[\s\S]*?infantrySquadOffsets/,
  'suppression marks should accept the AT-gun crew formation without changing normal infantry placement');
