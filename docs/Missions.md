...
任务编辑器将城市作为独立地形分类，且城市格固定使用夏季资源。地形代码如下：

| 代码 | 编辑器类型 | 运行时规则 |
|------|------------|------------|
| `u` | 城市地面 | 普通可通行地面 |
| `ur` | 城市道路 | 与普通道路规则相同，`rd` 六方向连接规则不变 |
| `ui` | 不可破坏建筑 | 编辑器仅一个类型；载入任务时从 8 种外观中随机分配；车辆不可进入、阻挡视线，地形判定参考森林 |
| `ud` | 可破坏建筑 | 编辑器仅一个类型；载入任务时从 4 套完整/受损/废墟外观中随机分配；结构值 2 |

同类建筑会先用完全部式样再开始重复，并避免两轮交界处连续出现相同式样。可破坏建筑只接受 HE：HE 威力 1–2 造成 1 点结构伤害，3–4 造成 2 点；向格内单位发射 HE 时同时结算建筑结构伤害，且不在战斗结果面板单列。建筑结构归零后变为废墟，规则参考泥地，坦克可以进入。完整建筑阻挡穿越该格的视线，格内步兵获得建筑掩护 `+1`。
