...
async function main(){
 ...
 const scale=514/1253,dx=230-525*scale,dy=164-386*scale-0;
 const artRes=await sharp(path.join(dir,'tiger-normal-preview-v6-aligned.png')).resize({width:Math.round(1949*scale)}).ensureAlpha().raw().toBuffer({resolveWithObject:true});
 ...
 fs.writeFileSync(path.join(dir,'comparison-overlay-v6.png'),o);
 ...
 fs.writeFileSync('C:/Users/Administrator/.codex/visualizations/2026/09/08/01a080f0-6d5a-7042-ae40-b77e02f90513/tiger-overlay-v6.html',html);
 fs.writeFileSync(path.join(dir,'comparison-alignment-v6.json'),JSON.stringify({scale,dx,dy,cropTop:370,referenceAnchors:[[230,534],[744,534]],artAnchors:[[525,386],[1778,386]],method:'Manual corresponding deck-edge anchors; uniform scaling and translation only; estimates, not CAD measurements'},null,2));
